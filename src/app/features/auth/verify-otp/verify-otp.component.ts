...
export class VerifyOtpComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        const pendingEmail = this.auth.getPendingEmail();
        this.email.set(pendingEmail);

        // If there is no pending registration, send the user back to registration.
        if (!pendingEmail) {
            this.errorMessage.set('No pending registration found. Please register again.');
    ...
    verify(code = this.code()): void {
         ...
         * 2. Read the generated OTP from sessionStorage.
         * 3. Compare the entered OTP with the stored OTP.
         * 4. If correct, create/login the user.
         * 5. Remove the OTP.
         */
        this.auth
            .verifyOtp(code)
            .subscribe({
                ...
                next: () => {
                     ...
                     * AuthService has already created the session and cleaned up data.
                ...
                error: (err: Error) => {
                    ...
                    this.errorMessage.set(
                        err.message || 'Incorrect OTP. Please try again.'
                    );
    ...
    resend(): void {
         ...
         * 2. Replace the old OTP in sessionStorage.
        ...
        this.auth
            ...
            .subscribe({
                ...
                next: () => {
                    ...
                    console.log('A new OTP has been generated.');
                },
                // -----------------------------------
                // RESEND ERROR
                // -----------------------------------
